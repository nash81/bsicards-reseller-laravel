        ...
        <div class="login-content">
            ...
            <div class="auth-body">
                <div class="auth-header">
                    <h2 class="auth-title">{{ __('Sign in to admin') }}</h2>
                    <p class="auth-subtitle">{{ __('Use your admin credentials to access the dashboard.') }}</p>
                </div>
                ...
                <form action="{{ route('admin.login') }}" method="post">
                    ...
                    <div class="single-box">
                        <label class="box-label">{{ __('Password') }}</label>
                        <input type="password" name="password" class="box-input" required />
                    </div>

                    <div class="single-box auth-actions">
                        <button class="site-btn primary-btn" type="submit">{{ __('Continue') }}</button>
                        <a href="{{route('admin.forget.password.now')}}" class="link">{{ __('Forgot your password?') }}</a>
                    </div>
                </form>
            </div>
        </div>
