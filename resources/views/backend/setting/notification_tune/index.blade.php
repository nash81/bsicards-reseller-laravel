...
@section('setting-content')
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 notification-tune-listview">
        <div class="site-card">
            ...
            <div class="site-card-body p-0">
                <div class="list-group list-group-flush">
                    @foreach($set_tunes as $set_tune)
                        <div class="list-group-item d-flex align-items-center justify-content-between py-3 tune-row-item">
                            <div class="d-flex align-items-center gap-3 flex-grow-1">
                                <div class="tune-icon-wrapper" style="width: 48px; height: 48px; border-radius: 8px; background: #f6f9fc; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                    <img src="{{ asset($set_tune->icon) }}" alt="{{ $set_tune->name }}" style="max-width: 32px; max-height: 32px;"/>
                                </div>
                                <div class="flex-grow-1">
                                    <h6 class="mb-0" style="font-size: 15px; font-weight: 600; color: #0a2540;">{{ $set_tune->name }}</h6>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-2 tune-action-buttons">
                                <button type="button" value="{{ $set_tune->id }}" data-tune-preview="{{ asset($set_tune->tune) }}" class="site-btn-sm primary-btn audioPlay tune-play-btn">
                                ...
                                @if($set_tune->status == true)
                                    <a href="{{ route('admin.settings.notification.tune.status', $set_tune->id) }}" class="site-btn-sm green-btn tune-status-btn">
                                        <i data-lucide="check"></i>{{ __('Active') }}
                                    </a>
                                @else
                                    <a href="{{ route('admin.settings.notification.tune.status', $set_tune->id) }}" class="site-btn-sm red-btn tune-status-btn">
                                        <i data-lucide="x"></i>{{ __('Inactive') }}
                                    </a>
                                @endif
                            </div>
                        </div>
