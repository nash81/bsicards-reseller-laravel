...
<script>
    ...
    // Color Switcher (legacy toggle)
    $(document).on('click', '.color-switcher', function () {
        $("body").toggleClass("dark-theme");
        $.get('{{ route("mode-theme") }}');
    });

    // Explicit Theme Dropdown (user dashboard)
    $(document).on('click', '.theme-mode-option', function () {
        var targetMode = $(this).data('theme');
