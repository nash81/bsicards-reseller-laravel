<aside class="user-sidebar">
    @php
    $height = setting('site_logo_height','global') == 'auto' ? '32' : setting('site_logo_height','global');
    @endphp
    <div class="site-logo">
        <a href="{{route('home')}}" class="logo">
            <img src="{{ asset(setting('site_logo','global')) }}" style="height:{{ $height }}px; width:auto; max-width:100%" alt="{{ auth()->user()->full_name }}">
        </a>
    </div>
    @php
    ...
    <nav class="user-nav">
        <ul>
            ...
            @include('frontend::include.__menu-item',['navigation' => $navigation])
            @elseif ($navigation->type == 'card' && setting('user_deposit', 'permission') && auth()->user()->deposit_status)
            @include('frontend::include.__menu-item',['navigation' => $navigation])
