                ...
                <div class="row">
                    @foreach($user_kycs as $kyc)
                        ...
                        @class([
                            ...
                            <div class="contents">
                                <div class="content">
                                    <strong>{{ $kyc->kyc?->name ?? $kyc->type }}</strong> is
                                    @if($kyc->status == 'pending')
                                        <span class="badge pending">{{ ucfirst($kyc->status) }}</span>
                                    @elseif($kyc->status == 'rejected')
                                        <span class="badge rejected">{{ ucfirst($kyc->status) }}</span>
                                    @elseif($kyc->status == 'approved')
                                        <span class="badge approved">{{ ucfirst($kyc->status) }}</span>
                                    @endif
